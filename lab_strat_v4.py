...
class lab_strat_v4(IStrategy):
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                    ((dataframe[f'buy_ema_{self.buy_ema_period.value}_1d'] < dataframe['close_1d']) & (dataframe['ema_ok'] > 0) & (dataframe['obv_ema'].rolling(self.buy_obv1_ema.value).mean() < dataframe['obv_ema'])) |
                    (qtpylib.crossed_above(dataframe['close'], dataframe[f'ema_{self.buy_ema_test.value}']) & (dataframe['obv_ema'].rolling(self.buy_obv2_ema.value).mean() < dataframe['obv_ema'])) |
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                ...
                ((dataframe['obv_ema'].rolling(self.sell_obv_ema.value).mean() < dataframe['obv_ema']) & (qtpylib.crossed_below(dataframe['close'], dataframe[f'sell_ema_{self.sell_ema_period.value}'])))
            ),
            'sell'] = 1
        return dataframe
